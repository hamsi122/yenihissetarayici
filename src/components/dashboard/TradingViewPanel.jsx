...
const resolveTradingViewSymbol = (symbol = "AAPL") => {
  ...
  // Borsa öneki vermeden gönderildiğinde TradingView sembolü doğru borsaya eşliyor
  // (NASDAQ/NYSE ayrımı otomatik çözülür). Nokta gösterimi ise BRK-B → BRK.B içindir.
...
export const TradingViewPanel = ({ signal, scannerState }) => {
  ...
  const isBist = symbol.endsWith(".IS") || signal?.market === "BIST";
  ...
  return (
    <div className="space-y-4" data-testid="tradingview-panel-container">
      <Card className="border-border/70 bg-card/45 backdrop-blur-md" data-testid="tradingview-widget-card">
        ...
        <CardContent className="space-y-3 p-4">
          ...
          <div className="aspect-[4/3] overflow-hidden rounded-sm border border-border/70" data-testid="tradingview-widget-frame-wrapper">
            {isBist ? (
              <div
                className="flex h-full w-full items-center justify-center border border-destructive/50 bg-destructive/10 p-4 text-center"
                data-testid="tradingview-bist-notice"
              >
                <p className="text-sm font-semibold text-destructive">
                  TradingView koşulları gereği BIST hisseleri için grafik kullanılamıyor
                </p>
              </div>
            ) : (
              <iframe
      ...
      <div className="pb-1 text-center" data-testid="admin-login-link-row">
        <a
          href="/admin"
          className="text-xs font-medium text-muted-foreground underline underline-offset-4 transition-colors hover:text-primary"
